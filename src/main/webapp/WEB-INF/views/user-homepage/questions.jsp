								...
								<thead>
									<tr><th></th>
							            ...
							            <c:forEach items="${ansList}" var="answer">
							            	<th style="width: 10%"><h4><small><c:out value="${answer.text}" /></small></h4></th>
							            </c:forEach>
							        </tr>
								</thead>
								 <tbody>
									<c:forEach items="${quesList}" var="question" varStatus="row">
										<tr><td>${question.id}</td>
											...
											<c:forEach items="${ansList}" var="answer">
												<td class="col-md-1"><form:radiobutton path="responseList[${row.index}].answer.value" value="${answer.id}"/></td>
						                	</c:forEach>
						                </tr>
									</c:forEach>
								 <tbody>
